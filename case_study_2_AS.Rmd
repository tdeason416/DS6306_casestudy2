...
for (colval in names(num_obs)){
  percent_pos[colval] = find_percent(data_binned, sub_df, colval, num_obs[colval])}
```
...
* Unsuprisingly, our mean value for percent_pos is exactly in line with the overall percent who quit, but the median value is notably lower, and we seem to have a small cluster of events hovering around .33% quit; which is approximitly double the standard rate

* Let's take a closer look at some of those varaibles

```{r}
## move all our data into a dataframe of percent_pos verses number of observations
...
#thirtyfive <- data_binned[,colnames(subset(high_prob, percent_pos > 0.33))]
...
over_34 <- subset(high_prob, percent_pos > .34)
```
